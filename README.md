...
The selected translation/reconstruction scopes are complete and CI-validated from **Minecraft 1.8 through Minecraft 1.21.7** (**G1-G43**).
...
**G43 / Minecraft 1.21.7 / JEI 23.1.0** is pinned to JEI commit `ee33b5d69f6cf9167c32c2e84fdc69fa1b008440`, whose direct successor is the Minecraft 1.21.8 port. It contains 291 English keys (285 normal + 6 debug), keeps the 90-language selected scope, and reconstructs 65 addon/full-override locales plus 23 missing-key-only upstream supplements; `en_us` and `zh_cn` are complete selected upstream locales. G42→G43 preserves all 289 previous key/value meanings and adds `gui.jei.category.grindstone` plus the historically reintroduced same-key/same-English `gui.jei.category.grindstone.experience`. The pinned upstream Ukrainian file remains malformed JSON, so `uk_ua` remains an explicit valid full repair override. Full reconstruction QA is green, and the deterministic NeoForge candidate build/inspection produced 88 language resources with SHA-256 `d6c24f20e8f650c3ee6e52c0c7d6438f2c3972e2271d077fd93802703de82dbd`.
