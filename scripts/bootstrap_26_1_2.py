#!/usr/bin/env python3
"""Freeze deterministic G50 / Minecraft 26.1.2 source, ownership, diff, and reuse policy."""
from __future__ import annotations

import json
from pathlib import Path

import audit_26_1_2 as audit
...
def main() -> int:
    ...
    audit.assert_single_parent(first_patch, audit.BASE_ENDPOINT, "26.1.2 support commit")
    branch = audit.fetch_json(audit.BRANCH_API)
    if branch.get("commit", {}).get("sha") != audit.PINNED_COMMIT:
        raise ValueError("G50 maintained-branch snapshot moved; re-audit before freezing")
    ...
    if (len(target), len(normal), len(target) - len(normal)) != (334, 328, 6):
        raise ValueError("unexpected G50 English counts")
    if (len(unchanged), len(added), len(removed), len(changed)) != (294, 34, 9, 6):
        raise ValueError("G49 -> G50 frozen English delta changed")
    ...
    for token in (
        ...
        if token not in props:
            raise ValueError(f"G50 metadata missing {token}")
    ...
    if malformed:
        raise ValueError(f"G50 selected upstream JSON unexpectedly malformed: {malformed}")
    if (len(addon_full), len(incomplete), len(complete)) != (63, 26, 1) or complete != ["en_us"]:
        raise ValueError(f"unexpected G50 ownership: {len(addon_full)}/{len(incomplete)}/{len(complete)} {complete}")
    if "fil_ph" not in incomplete:
        raise ValueError("G50 expects fil_ph to move from full addon ownership to upstream supplement ownership")
    ...
    if live_added or live_removed or selected - target_codes:
        raise ValueError(
            f"G50 Minecraft language membership changed: added={live_added} removed={live_removed} "
    ...
    if len(fallback_locales) != 30:
        raise ValueError(f"G50 documented fallback ownership changed: {len(fallback_locales)}")
    ...
    for locale in incomplete:
        ...
        unsafe = [
            ...
            if not g49.preserves_runtime_literals(target[key], values[key])
        ]
        if unsafe:
    ...
    write_json(DIFF_PATH, {
        ...
        "added_keys": added,
        "added_english_values": {k: target[k] for k in added},
        "removed_keys": removed,
        "changed_english_values": {k: {"from": base[k], "to": target[k]} for k in changed},
        "note": "Only the 294 exact same-key/same-English G49 meanings are eligible for direct translation reuse. New and changed meanings use pinned upstream translations where owned, otherwise exact target-English fallback until separately reviewed.",
    })
    ...
    scope = {
        ...
        "base_scope_manifest": "upstream/minecraft-26.1.1-language-scope.json",
        "raw_language_count": len(target_codes),
        "selected_scope_count": 90,
        "selected_scope_changed_from_26.1.1": False,
        ...
        "downloaded": {
            ...
            "live_language_asset_additions_from_26.1.1": live_added,
            "live_language_asset_removals_from_26.1.1": live_removed,
        },
        ...
        "translation_delta": {
            "base_generation": "g49-mc26.1.1",
            "unchanged_key_and_english_value_count": len(unchanged),
            "added_key_count": len(added),
            "removed_key_count": len(removed),
            "changed_english_value_count": len(changed),
            "reuse_exact_g49_for_identical_meanings_only": True,
            "novel_or_changed_meanings_default_to_exact_target_english_when_not_upstream_owned": True,
        },
        ...
        "upstream_supplement_policy": {
            ...
            "reuse_exact_g49_combined_value_only_for_same_key_same_english": True,
        ...
        "source_audit": "upstream/minecraft-26.1.2-language-audit.json",
        "english_diff": "upstream/diffs/26.1.1-to-26.1.2.json",
    }
    ...
    write_json(AUDIT_PATH, {
        ...
        "endpoint_resolution": {
            "first_26_1_2_commit": audit.FIRST_PATCH_COMMIT,
            "base_26_1_1_endpoint": audit.BASE_ENDPOINT,
            "maintained_branch": audit.MAINTAINED_BRANCH,
            "pinned_26_1_2_snapshot": audit.PINNED_COMMIT,
            "note": "Minecraft 26.1.2 remains maintained on JEI branch 26.1. G50 pins the audited branch head for reproducibility instead of claiming a historical final endpoint.",
        },
        "build_metadata": {
            "neoforge": "26.1.2.99",
    ...
    write_json(POLICY_PATH, {
        ...
        "translation_reuse": {
            ...
            "eligible_unchanged_semantics": len(unchanged),
            "novel_or_changed_semantics": len(added) + len(changed),
            "cross_key_reuse_allowed": False,
            "placeholder_and_technical_literal_safety_required": True,
            "prefer_target_english_if_reused_value_is_unsafe_or_semantics_changed": True,
        },
    ...
    print("PASS: froze G50 Minecraft 26.1.2 manifests")
    print(f"English: {len(target)} total / {len(normal)} normal / {len(target)-len(normal)} debug")
    print(f"G49 -> G50: {len(unchanged)} unchanged + {len(added)} added + {len(removed)} removed + {len(changed)} changed")
    print(f"Ownership: {len(addon_full)} full + {len(incomplete)} supplements + {len(complete)} complete upstream = 90")
    print(f"Upstream literal-safety override keys: {override_count}")
    return 0
