#!/usr/bin/env python3
"""Exploratory audit for final Minecraft 1.21.1 / JEI 19.21.1 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.21-language-scope.json"
PINNED_COMMIT = "28eb51f58d2798512a2ef75cf8b29189228573ad"
...
UPSTREAM_LOCALES = (
    "ar_sa", "bg_bg", "cs_cz", "de_de", "el_gr", "en_us", "es_es", "fi_fi", "fr_fr",
    "he_il", "hu_hu", "id_id", "it_it", "ja_jp", "kk_kz", "ko_kr", "lt_lt", "no_no",
    "pl_pl", "pt_br", "pt_pt", "ru_ru", "sv_se", "tr_tr", "uk_ua", "vi_vn", "zh_cn", "zh_tw",
)


def fetch_bytes(url: str, timeout: int = 30) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G39-audit"})
...
def main() -> int:
    ...
    for token in (
        "modJavaVersion=21", "minecraftVersion=1.21.1", "neoforgeVersion=21.1.116",
        "specificationVersion=19.21.1",
    ):
        if token not in props:
            errors.append(f"pinned G39 gradle.properties missing {token}")
    ...
    if len(inherited) != 90:
        errors.append(f"expected G38 selected scope 90, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.21"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.21.1"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.21/1.21.1 missing from Mojang version manifest")
    ...
    if inherited - target_codes:
        errors.append(f"selected G38 language codes absent from 1.21.1 asset pool: {sorted(inherited-target_codes)}")
    ...
    print("Minecraft 1.21.1 / JEI 19.21.1 final localization exploratory audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print("Endpoint: parent of Minecraft 1.21.4 port c0d0367841b16fa3a9567c3d93172cbd1f1b578c")
    print("No separate mainline JEI 1.21.2 or 1.21.3 port exists on this boundary.")
    print("Build: Minecraft 1.21.1 / NeoForge 21.1.116 / Java 21")
    print(f"G38 English: {len(base)} total / {len(base_normal)} normal / {len(base)-len(base_normal)} debug")
    print(f"G39 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G38 -> G39 unchanged: {len(unchanged)}")
    print(f"G38 -> G39 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G38 -> G39 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G38 -> G39 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    for key in changed:
        print(f"  changed {key}: {base[key]!r} -> {target[key]!r}")
    if base_meta and target_meta:
        ba, ta = base_meta["assetIndex"], target_meta["assetIndex"]
        print(f"Minecraft 1.21 asset index: id={ba.get('id')} sha1={ba.get('sha1')}")
        print(f"Minecraft 1.21.1 asset index: id={ta.get('id')} sha1={ta.get('sha1')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1')}")
    print(f"Minecraft live language asset-file codes: 1.21={len(base_codes)} 1.21.1={len(target_codes)}")
    ...
    print(f"Newly JEI-upstream selected locales from G38 addon-full ({len(newly_upstream)}): {', '.join(newly_upstream) or '(none)'}")
    print("Upstream completeness against normal G39 target keys:")
    for locale in UPSTREAM_LOCALES:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.21.1 / JEI 19.21.1 exploratory audit")
    return 0
