#!/usr/bin/env python3
"""Exploratory audit for final Minecraft 1.20.6 / JEI 18.0.0 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.20.4-language-scope.json"
PINNED_COMMIT = "7cc7d59043102c4843d422138dd74133edcd7b3c"
...
def fetch_bytes(url: str, timeout: int = 30) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G37-audit"})
...
def main() -> int:
    ...
    if (len(base), len(target), len(target_normal)) != (157, 157, 151):
    ...
    if (len(unchanged), len(added), len(removed), len(changed)) != (157, 0, 0, 0):
        errors.append("expected all 157 G36 English key/value pairs to remain unchanged")

    props = fetch_text(f"{RAW_ROOT}/gradle.properties")
    forge_build = fetch_text(f"{RAW_ROOT}/Forge/build.gradle.kts")
    for token in (
        "modJavaVersion=21", "minecraftVersion=1.20.6", "forgeVersion=50.1.3",
        "parchmentVersionForge=1.20.6-2024.06.02-1.20.6", "specificationVersion=18.0.0",
    ):
        if token not in props:
            errors.append(f"pinned G37 gradle.properties missing {token}")
    if 'languageVersion.set(JavaLanguageVersion.of(modJavaVersion))' not in forge_build:
        errors.append("pinned G37 Forge build metadata no longer confirms Java toolchain")
    if 'mappings("parchment", parchmentVersionForge)' not in forge_build:
        errors.append("pinned G37 Forge build metadata no longer confirms Parchment mappings")
    ...
    if len(inherited) != 90:
        errors.append(f"expected G36 selected scope 90, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.20.4"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.20.6"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.20.4/1.20.6 missing from Mojang version manifest")
    ...
    if inherited - target_codes:
        errors.append(f"selected G36 language codes absent from 1.20.6 asset pool: {sorted(inherited-target_codes)}")
    ...
    print("Minecraft 1.20.6 / JEI 18.0.0 final localization exploratory audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print("Build: Minecraft 1.20.6 / Forge 50.1.3 / Parchment 1.20.6-2024.06.02-1.20.6 / Java 21")
    print(f"G36 English: {len(base)} total / {len(base_normal)} normal / {len(base)-len(base_normal)} debug")
    print(f"G37 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G36 -> G37 unchanged: {len(unchanged)}")
    print(f"G36 -> G37 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G36 -> G37 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G36 -> G37 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    if base_meta and target_meta:
        ba, ta = base_meta["assetIndex"], target_meta["assetIndex"]
        print(f"Minecraft 1.20.4 asset index: id={ba.get('id')} sha1={ba.get('sha1')}")
        print(f"Minecraft 1.20.6 asset index: id={ta.get('id')} sha1={ta.get('sha1')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1')}")
    print(f"Minecraft live language asset-file codes: 1.20.4={len(base_codes)} 1.20.6={len(target_codes)}")
    ...
    print("Upstream completeness against normal G37 target keys:")
    for locale in UPSTREAM_LOCALES:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.20.6 / JEI 18.0.0 exploratory audit")
    return 0
