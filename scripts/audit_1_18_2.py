#!/usr/bin/env python3
"""Exploratory audit for final Minecraft 1.18.2 / JEI 10.1.0 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.18.1-language-scope.json"
PINNED_COMMIT = "530ef6c8d604370bef850f3656a28beab56cbfba"
...
def fetch_bytes(url: str, timeout: int = 30) -> bytes:
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "JEI-Translation-Expansion-G27-audit"},
    )
...
def main() -> int:
    ...
    if stored_target != remote_target:
        errors.append("stored G27 English source differs from pinned JEI 10.1.0 endpoint")
    ...
    reviewed_normal = sorted(
        k for k in set(added) | set(changed)
        if not k.startswith(DEBUG_PREFIX)
    )
    ...
    for token in (
        ...
        "parchmentVersionForge=2022.03.13-1.18.2",
        "specificationVersion=10.1.0",
    ):
        if token not in props:
            errors.append(f"pinned G27 gradle.properties missing {token}")
    if 'languageVersion.set(JavaLanguageVersion.of(modJavaVersion))' not in forge_build:
        errors.append("pinned G27 Forge build metadata no longer confirms Java toolchain")
    if 'mappings("parchment", parchmentVersionForge)' not in forge_build:
        errors.append("pinned G27 Forge build metadata no longer confirms Parchment mappings")
    ...
    if len(inherited) != 86:
        errors.append(f"expected G26 selected scope 86, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.18.1"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.18.2"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.18.1/1.18.2 missing from Mojang version manifest")
    ...
    print("Minecraft 1.18.2 / JEI 10.1.0 final localization exploratory audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print("Build: Minecraft 1.18.2 / Forge 40.0.24 / Parchment 2022.03.13-1.18.2 / Java 17")
    print(f"G26 English: {len(base)} total / {len(base_normal)} normal / {len(base)-len(base_normal)} debug")
    print(f"G27 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G26 -> G27 unchanged: {len(unchanged)}")
    print(f"G26 -> G27 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G26 -> G27 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G26 -> G27 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    ...
    print("Upstream completeness against normal G27 target keys:")
    for locale in upstream_locales:
    ...
    if base_meta and target_meta:
        ...
        print(f"Minecraft 1.18.1 asset index: id={ba.get('id')} sha1={ba.get('sha1')} url={ba.get('url')}")
        print(f"Minecraft 1.18.2 asset index: id={ta.get('id')} sha1={ta.get('sha1')} url={ta.get('url')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1')}")
    print(f"Minecraft language asset-file codes: 1.18.1={len(base_codes)} 1.18.2={len(target_codes)}")
    print(f"Added asset-file codes since 1.18.1 ({len(added_mc)}): {', '.join(added_mc) or '(none)'}")
    print(f"Removed asset-file codes since 1.18.1 ({len(removed_mc)}): {', '.join(removed_mc) or '(none)'}")
    ...
    print(f"Inherited selected codes absent from 1.18.2 ({len(inherited_missing)}): {', '.join(inherited_missing) or '(none)'}")
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.18.2 / JEI 10.1.0 exploratory audit")
    return 0
