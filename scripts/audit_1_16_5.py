#!/usr/bin/env python3
"""Audit final Minecraft 1.16.5 / JEI 7.7.1 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.16.4-language-scope.json"
DIFF_PATH = ROOT / "upstream" / "diffs" / "1.16.4-to-1.16.5.json"
PINNED_COMMIT = "f6bd6ea033084f3d18ad256c9921641dcbd0330f"
...
def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "JEI-Translation-Expansion-G23-audit",
...
def main() -> int:
    ...
    if stored_target != remote_target:
        errors.append("stored G23 English source differs from pinned JEI 7.7.1 endpoint")
    ...
    if (len(unchanged), len(added), len(removed), len(changed)) != (
        ...
        errors.append("frozen G22->G23 diff counts differ from pinned endpoint")
    if added != sorted(diff["added_keys"]) or removed != sorted(diff["removed_keys"]) or changed != sorted(diff["changed_english_values"]):
        errors.append("frozen G23 key delta differs from pinned endpoint")

    props = fetch_text(f"{RAW_ROOT}/gradle.properties")
    build = fetch_text(f"{RAW_ROOT}/build.gradle")
    for token in (
        "mcversion=1.16.5", "forgeversion=36.2.0",
        "version_major=7", "version_minor=7", "version_patch=1",
    ):
    ...
    try:
        ...
        errors.append(f"failed to discover pinned G23 upstream locale inventory: {exc}")
        upstream_locales = ()
    ...
    if len(inherited) != 88:
        errors.append(f"expected G22 selected scope 88, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.16.4"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.16.5"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.16.4/1.16.5 missing from Mojang version manifest")
    ...
    print("Minecraft 1.16.5 / JEI 7.7.1 final localization audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print("Build: Minecraft 1.16.5 / Forge 36.2.0 / mappings official 1.16.5 / Java 8")
    print(f"G22 English: {len(base)} total")
    print(f"G23 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G22 -> G23 unchanged: {len(unchanged)}")
    print(f"G22 -> G23 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G22 -> G23 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G22 -> G23 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    print(f"JEI upstream JSON locales ({len(upstream_locales)}): {', '.join(upstream_locales)}")
    print("Upstream completeness against normal G23 target keys:")
    for locale in upstream_locales:
    ...
    if base_meta and target_meta:
        ...
        print(f"Minecraft 1.16.4 asset index: id={ba.get('id')} sha1={ba.get('sha1')} url={ba.get('url')}")
        print(f"Minecraft 1.16.5 asset index: id={ta.get('id')} sha1={ta.get('sha1')} url={ta.get('url')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1')}")
    print(f"Minecraft language codes: 1.16.4={len(base_codes)} 1.16.5={len(target_codes)}")
    print(f"Added codes since 1.16.4 ({len(added_mc)}): {', '.join(added_mc) or '(none)'}")
    print(f"Removed codes since 1.16.4 ({len(removed_mc)}): {', '.join(removed_mc) or '(none)'}")
    print(f"Inherited selected codes absent from 1.16.5 ({len(inherited_missing)}): {', '.join(inherited_missing) or '(none)'}")
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.16.5 / JEI 7.7.1 audit")
    return 0
