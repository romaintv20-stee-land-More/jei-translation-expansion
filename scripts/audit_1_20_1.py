#!/usr/bin/env python3
"""Exploratory audit for final Minecraft 1.20.1 / JEI 15.2.0 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.20-language-scope.json"
PINNED_COMMIT = "eecef8ae335701b97a6918e20b4dd87966a46dfa"
...
def fetch_bytes(url: str, timeout: int = 30) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G34-audit"})
...
def main() -> int:
    ...
    for token in (
        "modJavaVersion=17", "minecraftVersion=1.20.1", "forgeVersion=47.0.1",
        "parchmentVersionForge=1.19.3-2023.03.12-1.20.1", "specificationVersion=15.2.0",
    ):
        if token not in props:
            errors.append(f"pinned G34 gradle.properties missing {token}")
    if 'languageVersion.set(JavaLanguageVersion.of(modJavaVersion))' not in forge_build:
        errors.append("pinned G34 Forge build metadata no longer confirms Java toolchain")
    if 'mappings("parchment", parchmentVersionForge)' not in forge_build:
        errors.append("pinned G34 Forge build metadata no longer confirms Parchment mappings")
    ...
    if len(inherited) != 90:
        errors.append(f"expected G33 selected scope 90, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.20"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.20.1"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.20/1.20.1 missing from Mojang version manifest")
    ...
    if inherited - target_codes:
        errors.append(f"selected G33 language codes absent from 1.20.1 asset pool: {sorted(inherited-target_codes)}")
    ...
    print("Minecraft 1.20.1 / JEI 15.2.0 final localization exploratory audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print("Build: Minecraft 1.20.1 / Forge 47.0.1 / Parchment 1.19.3-2023.03.12-1.20.1 / Java 17")
    print(f"G33 English: {len(base)} total / {len(base_normal)} normal / {len(base)-len(base_normal)} debug")
    print(f"G34 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G33 -> G34 unchanged: {len(unchanged)}")
    print(f"G33 -> G34 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G33 -> G34 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G33 -> G34 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    if base_meta and target_meta:
        ba, ta = base_meta["assetIndex"], target_meta["assetIndex"]
        print(f"Minecraft 1.20 asset index: id={ba.get('id')} sha1={ba.get('sha1')}")
        print(f"Minecraft 1.20.1 asset index: id={ta.get('id')} sha1={ta.get('sha1')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1')}")
    print(f"Minecraft live language asset-file codes: 1.20={len(base_codes)} 1.20.1={len(target_codes)}")
    ...
    print("Upstream completeness against normal G34 target keys:")
    for locale in UPSTREAM_LOCALES:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.20.1 / JEI 15.2.0 exploratory audit")
    return 0
