#!/usr/bin/env python3
"""Audit final Minecraft 1.13.2 / JEI 5.0.0 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.13-language-scope.json"
PINNED_COMMIT = "2d16f4210cbae340ad76b483b4aa8b461561e86f"
...
def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G13-audit"})
...
def main() -> int:
    ...
    if len(inherited) != 83:
        errors.append(f"expected G12 selected scope of 83, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((item for item in manifest.get("versions", []) if item.get("id") == "1.13"), None)
    target_entry = next((item for item in manifest.get("versions", []) if item.get("id") == "1.13.2"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.13/1.13.2 missing from Mojang version manifest")
    ...
    print("Minecraft 1.13.2 / JEI 5.0.0 final localization audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print(f"G12 English: {len(base)} total")
    print(f"G13 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G12 -> G13 unchanged: {len(unchanged)}")
    print(f"G12 -> G13 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G12 -> G13 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G12 -> G13 changed ({len(changed)}):")
    for key in changed:
        print(f"  {key}: {base[key]!r} -> {target[key]!r}")

    print(f"JEI upstream JSON locales ({len(upstream_locales)}): {', '.join(upstream_locales)}")
    print("Upstream completeness against normal G13 target keys:")
    for locale in upstream_locales:
    ...
    if base_meta and target_meta:
        ...
        print(f"Minecraft 1.13 asset index: id={ba.get('id')} sha1={ba.get('sha1')} url={ba.get('url')}")
        print(f"Minecraft 1.13.2 asset index: id={ta.get('id')} sha1={ta.get('sha1')} url={ta.get('url')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1') and ba.get('url') == ta.get('url')}")
    print(f"Minecraft language codes: 1.13={len(base_codes)} 1.13.2={len(target_codes)}")
    print(f"Added codes since 1.13 ({len(added_mc)}): {', '.join(added_mc) or '(none)'}")
    print(f"Removed codes since 1.13 ({len(removed_mc)}): {', '.join(removed_mc) or '(none)'}")
    print(f"Inherited selected codes absent from 1.13.2 ({len(inherited_missing)}): {', '.join(inherited_missing) or '(none)'}")
    print(f"Inherited selected codes still present: {len(inherited_present)}")
    print(f"Minecraft 1.13.2 codes outside inherited scope ({len(new_candidates)}):")
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.13.2 / JEI 5.0.0 audit")
    return 0
