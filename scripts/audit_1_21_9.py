#!/usr/bin/env python3
"""Audit the final Minecraft 1.21.9 / JEI 25.0.1 endpoint before freezing G45 manifests."""
...
PINNED_COMMIT = "bdfdb4c09026c4fb488805ff729c66ae48ede875"
NEXT_PORT_COMMIT = "0999689eb56a4bb3f7061af263de7aef387f0045"
...
def fetch_bytes(url: str) -> bytes:
    headers = {"User-Agent": "JEI-Translation-Expansion-G45-audit"}
...
def repair_uk_ua(text: str) -> str:
    ...
    if count != 1:
        raise ValueError("uk_ua missing-comma repair point changed")
...
def parse_locale(locale: str) -> tuple[dict[str, str], bool]:
    ...
    try:
        ...
        if locale != "uk_ua":
            raise
        return clean(json.loads(repair_uk_ua(text))), True
...
def main() -> int:
    ...
    if parents != [PINNED_COMMIT]:
        raise ValueError(f"1.21.10 port is not directly parented by G45 endpoint: {parents}")
    if "1.21.10" not in next_commit.get("commit", {}).get("message", ""):
        raise ValueError("next port commit no longer identifies Minecraft 1.21.10")
    ...
    for token in required:
        if token not in props:
            raise ValueError(f"pinned G45 gradle.properties missing {token}")
    ...
    if (len(unchanged), len(added), len(removed), len(changed)) != (297, 8, 8, 0):
        raise ValueError(f"unexpected G44 -> G45 semantic partition: {len(unchanged)}/{len(added)}/{len(removed)}/{len(changed)}")
    if set(added) != NEW_CATEGORY_KEYS or set(removed) != OLD_CATEGORY_KEYS:
    ...
    if malformed not in ({"uk_ua"}, set()):
        raise ValueError(f"unexpected malformed selected upstream set: {sorted(malformed)}")
    ...
    if selected - target_codes:
        raise ValueError(f"selected locales absent from Minecraft 1.21.9: {sorted(selected-target_codes)}")

    print("PASS: exploratory G45 Minecraft 1.21.9 / JEI 25.0.1 audit")
    print(f"Endpoint: {PINNED_COMMIT}")
    print(f"Next port: {NEXT_PORT_COMMIT} (Minecraft 1.21.10, direct child)")
