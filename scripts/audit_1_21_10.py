#!/usr/bin/env python3
"""Audit the final Minecraft 1.21.10 / JEI 26.2.0 endpoint before freezing G46 manifests."""
...
PINNED_COMMIT = "621ddf003a8eceffcba0fd808a955e280f87a4c0"
NEXT_PORT_COMMIT = "6b615d15ef776abf139339779985a91c59c9c324"
...
def fetch_bytes(url: str) -> bytes:
    headers = {"User-Agent": "JEI-Translation-Expansion-G46-audit"}
...
def main() -> int:
    ...
    if parents != [PINNED_COMMIT]:
        raise ValueError(f"1.21.11 port is not directly parented by G46 endpoint: {parents}")
    if "1.21.11" not in next_commit.get("commit", {}).get("message", ""):
        raise ValueError("next port commit no longer identifies Minecraft 1.21.11")
    ...
    for token in required:
        if token not in props:
            raise ValueError(f"pinned G46 gradle.properties missing {token}")
    ...
    if len(unchanged) + len(added) + len(changed) != len(target):
        raise ValueError("G46 semantic partition does not cover target keys")
    ...
    if selected - target_codes:
        raise ValueError(f"selected locales absent from Minecraft 1.21.10: {sorted(selected-target_codes)}")

    print("PASS: exploratory G46 Minecraft 1.21.10 / JEI 26.2.0 audit")
    print(f"Endpoint: {PINNED_COMMIT}")
    print(f"Next port: {NEXT_PORT_COMMIT} (Minecraft 1.21.11, direct child)")
