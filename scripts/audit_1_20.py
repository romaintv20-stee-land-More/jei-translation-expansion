#!/usr/bin/env python3
"""Exploratory audit for final Minecraft 1.20 / JEI 14.0.0 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.19.4-language-scope.json"
PINNED_COMMIT = "aa6e14229c0c44cd685ac6b4d1d7f513360da18a"
...
# Java Edition 1.20 added two primary real-world languages: Lao and Yakut.
# Their runtime locale codes are lo_la and sah_sah. They are added to the selected
# historical scope; later decoupled language-asset additions are not backported.
HISTORICAL_G33_LANGUAGE_ADDITIONS = {"lo_la", "sah_sah"}


def fetch_bytes(url: str, timeout: int = 30) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G33-audit"})
...
def main() -> int:
    ...
    if 'languageVersion.set(JavaLanguageVersion.of(modJavaVersion))' not in forge_build:
        errors.append("pinned G33 Forge build metadata no longer confirms Java toolchain")
    if 'mappings("parchment", parchmentVersionForge)' not in forge_build:
        errors.append("pinned G33 Forge build metadata no longer confirms Parchment mappings")
    ...
    if len(inherited) != 88:
        errors.append(f"expected G32 selected scope 88, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.19.4"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.20"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.19.4/1.20 missing from Mojang version manifest")
    ...
    if not HISTORICAL_G33_LANGUAGE_ADDITIONS <= target_codes:
        errors.append("historical G33 language additions are not present in the live 1.20 asset pool")

    effective_scope_codes = inherited | HISTORICAL_G33_LANGUAGE_ADDITIONS
    if len(effective_scope_codes) != 90:
        errors.append(f"expected G33 selected scope 90 after Lao/Yakut addition, got {len(effective_scope_codes)}")

    upstream_set = set(UPSTREAM_LOCALES)
    selected_upstream = sorted(effective_scope_codes & upstream_set)
    ...
    selected_full = sorted(effective_scope_codes - upstream_set)
    ...
    print(f"G32 -> G33 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G32 -> G33 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G32 -> G33 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    if base_meta and target_meta:
        ba, ta = base_meta["assetIndex"], target_meta["assetIndex"]
        print(f"Minecraft 1.19.4 asset index: id={ba.get('id')} sha1={ba.get('sha1')}")
        print(f"Minecraft 1.20 asset index: id={ta.get('id')} sha1={ta.get('sha1')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1')}")
    print(f"Minecraft live language asset-file codes: 1.19.4={len(base_codes)} 1.20={len(target_codes)}")
    print(f"Live asset additions ({len(live_asset_additions)}): {', '.join(sorted(live_asset_additions)) or '(none)'}")
    print("Historical G33 additions: lo_la (Lao), sah_sah (Yakut)")
    print(f"G33 selected scope: {len(effective_scope_codes)}")
    print("Upstream completeness against normal G33 target keys:")
    for locale in UPSTREAM_LOCALES:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.20 / JEI 14.0.0 exploratory audit")
    return 0
