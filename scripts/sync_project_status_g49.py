...
old_row = '| G47 | 1.21.11 | 27.38.0 | `4b6e473` | 334 | 90 | 63 | 26 | 1 | complete; maintained branch validated; packaging pending |'
new_rows = '\n'.join([
...
if '| G49 | 26.1.1 |' not in text:
    if old_row not in text:
        raise SystemExit('G47 table row anchor not found')
    text = text.replace(old_row, new_rows, 1)

stale = '- public candidate packaging remains a separate project step until the maintained-target metadata is merged and the candidate builder is validated'
replacement = '\n'.join([
...
if '### G48 — Minecraft 26.1 / JEI 29.2.0' not in text:
    if marker not in text:
        raise SystemExit('candidate-state heading anchor not found')
    milestones = '''
...
'''
    text = text.replace(marker, '\n' + milestones + '## Candidate packaging state\n', 1)

tail_marker = '## Candidate packaging state\n'
pos = text.find(tail_marker)
if pos < 0:
    raise SystemExit('candidate-state tail anchor not found')
tail = '''## Candidate packaging state
...
print('PASS: synchronized PROJECT_STATUS.md through G49')
