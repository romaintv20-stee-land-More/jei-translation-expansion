#!/usr/bin/env python3
"""Exploratory audit for final Minecraft 1.21 / JEI 19.8.2 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.20.6-language-scope.json"
PINNED_COMMIT = "02370235626ddb4e479e13e6cde9b6d5466cd5ff"
...
def fetch_bytes(url: str, timeout: int = 30) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G38-audit"})
...
def main() -> int:
    ...
    if (len(base), len(base_normal), len(target), len(target_normal)) != (157, 151, 176, 170):
        errors.append(f"unexpected English counts base={len(base)}/{len(base_normal)} target={len(target)}/{len(target_normal)}")
    if (len(unchanged), len(added), len(removed), len(changed)) != (156, 19, 0, 1):
        errors.append("expected G37->G38 delta 156 unchanged + 19 added + 0 removed + 1 changed")

    props = fetch_text(f"{RAW_ROOT}/gradle.properties")
    forge_build = fetch_text(f"{RAW_ROOT}/Forge/build.gradle.kts")
    for token in (
        "modJavaVersion=21", "minecraftVersion=1.21", "forgeVersion=51.0.31",
        "parchmentVersionForge=2024.07.28-1.21", "specificationVersion=19.8.2",
    ):
    ...
    if len(inherited) != 90:
        errors.append(f"expected G37 selected scope 90, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.20.6"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.21"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.20.6/1.21 missing from Mojang version manifest")
    ...
    if inherited - target_codes:
        errors.append(f"selected G37 language codes absent from 1.21 asset pool: {sorted(inherited-target_codes)}")
    ...
    print(f"G37 -> G38 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G37 -> G38 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G37 -> G38 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    for key in changed:
        print(f"  changed {key}: {base[key]!r} -> {target[key]!r}")
    if base_meta and target_meta:
        ba, ta = base_meta["assetIndex"], target_meta["assetIndex"]
        print(f"Minecraft 1.20.6 asset index: id={ba.get('id')} sha1={ba.get('sha1')}")
        print(f"Minecraft 1.21 asset index: id={ta.get('id')} sha1={ta.get('sha1')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1')}")
    print(f"Minecraft live language asset-file codes: 1.20.6={len(base_codes)} 1.21={len(target_codes)}")
    ...
    print("Upstream completeness against normal G38 target keys:")
    for locale in UPSTREAM_LOCALES:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.21 / JEI 19.8.2 exploratory audit")
    return 0
