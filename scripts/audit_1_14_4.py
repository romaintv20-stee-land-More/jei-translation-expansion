#!/usr/bin/env python3
"""Audit final Minecraft 1.14.4 / JEI 6.0.1 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.14.3-language-scope.json"
PINNED_COMMIT = "de8b6a10eba45899be1c62694c9f27a435ae8c2c"
...
def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G16-audit"})
...
def main() -> int:
    ...
    if len(inherited) != 91:
        errors.append(f"expected G15 selected scope of 91, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((item for item in manifest.get("versions", []) if item.get("id") == "1.14.3"), None)
    target_entry = next((item for item in manifest.get("versions", []) if item.get("id") == "1.14.4"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.14.3/1.14.4 missing from Mojang version manifest")
        base_meta = target_meta = {}; base_asset = target_asset = {}
    else:
        base_meta = fetch_json(base_entry["url"]); target_meta = fetch_json(target_entry["url"])
        base_asset = fetch_json(base_meta["assetIndex"]["url"]); target_asset = fetch_json(target_meta["assetIndex"]["url"])
    ...
    added_mc = sorted(target_codes - base_codes); removed_mc = sorted(base_codes - target_codes)
    inherited_missing = sorted(inherited - target_codes); inherited_present = inherited & target_codes
    ...
    print("Minecraft 1.14.4 / JEI 6.0.1 final localization audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print(f"G15 English: {len(base)} total")
    print(f"G16 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G15 -> G16 unchanged: {len(unchanged)}")
    print(f"G15 -> G16 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G15 -> G16 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G15 -> G16 changed ({len(changed)}):")
    for key in changed: print(f"  {key}: {base[key]!r} -> {target[key]!r}")
    print(f"JEI upstream JSON locales ({len(upstream_locales)}): {', '.join(upstream_locales)}")
    print("Upstream completeness against normal G16 target keys:")
    for locale in upstream_locales:
    ...
    if base_meta and target_meta:
        ba = base_meta.get("assetIndex", {}); ta = target_meta.get("assetIndex", {})
        print(f"Minecraft 1.14.3 asset index: id={ba.get('id')} sha1={ba.get('sha1')} url={ba.get('url')}")
        print(f"Minecraft 1.14.4 asset index: id={ta.get('id')} sha1={ta.get('sha1')} url={ta.get('url')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1') and ba.get('url') == ta.get('url')}")
    print(f"Minecraft language codes: 1.14.3={len(base_codes)} 1.14.4={len(target_codes)}")
    print(f"Added codes since 1.14.3 ({len(added_mc)}): {', '.join(added_mc) or '(none)'}")
    print(f"Removed codes since 1.14.3 ({len(removed_mc)}): {', '.join(removed_mc) or '(none)'}")
    print(f"Inherited selected codes absent from 1.14.4 ({len(inherited_missing)}): {', '.join(inherited_missing) or '(none)'}")
    ...
    if errors:
        ...
        return 1
    print("PASS: pinned final Minecraft 1.14.4 / JEI 6.0.1 audit")
    return 0
