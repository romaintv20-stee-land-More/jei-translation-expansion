#!/usr/bin/env python3
"""Validate G35 Minecraft 1.20.2 / JEI 16.0.0 source, scope and ownership."""
...
AUDIT_PATH = ROOT / "upstream" / "minecraft-1.20.2-language-audit.json"
...
BASE_SCOPE_PATH = ROOT / "upstream" / "minecraft-1.20.1-language-scope.json"
...
def main() -> int:
    ...
    if (len(base), len(target), len(normal)) != (156, 156, 150):
        errors.append("G35 English counts changed")
    if (len(unchanged), len(added), len(removed), len(changed)) != (156, 0, 0, 0):
        errors.append("G35 English semantic delta must remain 156 unchanged + 0 added + 0 removed + 0 changed")
    if diff["added_keys"] or diff["removed_keys"] or diff["changed_english_values"]:
        errors.append("G35 frozen diff must contain no semantic changes")
    ...
    if (len(full), len(complete), len(incomplete), len(selected)) != (68, 2, 20, 90):
        errors.append("G35 ownership partition must be 68 full + 2 complete + 20 supplements = 90")
    if complete != EXPECTED_COMPLETE or incomplete != EXPECTED_INCOMPLETE:
        errors.append("G35 upstream ownership differs from frozen audit")
    ...
    if selected != base_selected:
        errors.append("G35 selected language scope must match G34 exactly")
    if full != set(base_scope["addon_full_locales"]):
        errors.append("G35 addon-full ownership must match G34 exactly")
    if scope["new_selected_primary_languages"] or scope["removed_selected_languages"]:
        errors.append("G35 must not add or remove selected languages")
    ...
    try:
        remote_english = g35.fetch_upstream_json(g35.G35_COMMIT, "en_us")
        if remote_english != target:
            errors.append("stored G35 English source differs from pinned JEI 16.0.0")
    ...
    if endpoint["next_minecraft_port_commit"] != "14609665505e736cf4f30811a01fdb45adc40f37" or endpoint["next_minecraft_version"] != "1.20.4":
        errors.append("G35 endpoint boundary changed")
    build = audit["build_metadata"]
    if build["forge"] != "48.0.1" or build["mappings_version"] != "1.20.1-2023.09.03-1.20.2" or build["java_toolchain"] != "17":
        errors.append("G35 frozen build metadata changed")
    if not policy["translation_reuse"]["reuse_unchanged_g34_semantics"] or policy["translation_reuse"]["cross_key_reuse_allowed"]:
        errors.append("G35 policy must enforce exact G34 reuse and forbid cross-key reuse")

    if errors:
        print(f"FAIL: {len(errors)} Minecraft 1.20.2 source/scope validation error(s)")
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: Minecraft 1.20.2 / JEI 16.0.0 G35 source, scope and ownership QA")
    print("English: 156 keys / 150 normal / 6 debug; all 156 semantics unchanged")
    print("Selected language scope: 90, unchanged from G34")
    print("Ownership: 68 addon-full + 20 supplements + 2 complete upstream")
    print("No separate JEI 1.20.3 generation; next mainline port is Minecraft 1.20.4")
    return 0
