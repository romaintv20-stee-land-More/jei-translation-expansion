#!/usr/bin/env python3
"""Audit final Minecraft 1.16.2 / JEI 7.3.2 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.16.1-language-scope.json"
DIFF_PATH = ROOT / "upstream" / "diffs" / "1.16.1-to-1.16.2.json"
PINNED_COMMIT = "df46cefb0ade79851101fd32b065a34115fee70c"
...
def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G20-audit"})
...
def main() -> int:
    ...
    if stored_target != remote_target:
        errors.append("stored G20 English source differs from pinned JEI 7.3.2 endpoint")
    ...
    if (len(unchanged), len(added), len(removed), len(changed)) != (
        ...
        errors.append("frozen G19->G20 diff counts differ from pinned endpoint")
    if added != diff["added_keys"] or removed != diff["removed_keys"] or changed != sorted(diff["changed_english_values"]):
        errors.append("frozen G20 key delta differs from pinned endpoint")

    props = fetch_text(f"{RAW_ROOT}/gradle.properties")
    build = fetch_text(f"{RAW_ROOT}/build.gradle")
    for token in ("mcversion=1.16.2", "forgeversion=33.0.19", "mcp_mappings=20200723-1.16.1", "version_major=7", "version_minor=3", "version_patch=2"):
        if token not in props:
            errors.append(f"pinned G20 gradle.properties missing {token}")
    if "JavaVersion.VERSION_1_8" not in build or 'mappings channel: "snapshot"' not in build:
        errors.append("pinned G20 build metadata no longer confirms Java 8 + snapshot mappings")
    ...
    if len(inherited) != 88:
        errors.append(f"expected G19 selected scope 88, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.16.1"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.16.2"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.16.1/1.16.2 missing from Mojang version manifest")
    ...
    print("Minecraft 1.16.2 / JEI 7.3.2 final localization audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print("Build: Minecraft 1.16.2 / Forge 33.0.19 / mappings snapshot 20200723-1.16.1 / Java 8")
    print(f"G19 English: {len(base)} total")
    print(f"G20 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G19 -> G20 unchanged: {len(unchanged)}")
    print(f"G19 -> G20 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G19 -> G20 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G19 -> G20 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    print(f"JEI upstream JSON locales ({len(UPSTREAM_LOCALES)}): {', '.join(UPSTREAM_LOCALES)}")
    print("Upstream completeness against normal G20 target keys:")
    for locale in UPSTREAM_LOCALES:
    ...
    if base_meta and target_meta:
        ...
        print(f"Minecraft 1.16.1 asset index: id={ba.get('id')} sha1={ba.get('sha1')} url={ba.get('url')}")
        print(f"Minecraft 1.16.2 asset index: id={ta.get('id')} sha1={ta.get('sha1')} url={ta.get('url')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1')}")
    print(f"Minecraft language codes: 1.16.1={len(base_codes)} 1.16.2={len(target_codes)}")
    print(f"Added codes since 1.16.1 ({len(added_mc)}): {', '.join(added_mc) or '(none)'}")
    print(f"Removed codes since 1.16.1 ({len(removed_mc)}): {', '.join(removed_mc) or '(none)'}")
    print(f"Inherited selected codes absent from 1.16.2 ({len(inherited_missing)}): {', '.join(inherited_missing) or '(none)'}")
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.16.2 / JEI 7.3.2 audit")
    return 0
