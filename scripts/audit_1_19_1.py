#!/usr/bin/env python3
"""Exploratory audit for final Minecraft 1.19.1 / JEI 11.2.0 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.19-language-scope.json"
PINNED_COMMIT = "90b37d06031c8e113d51e747e3f532f4b9ed047b"
...
# Canonical upstream ownership inventory inherited from G28. kk_kz remains
# project-owned in the selected scope despite an upstream file being present.
UPSTREAM_LOCALES = (
...
def fetch_bytes(url: str, timeout: int = 30) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G29-audit"})
...
def main() -> int:
    ...
    if (len(base), len(target), len(target_normal)) != (154, 153, 147):
    ...
    if (len(unchanged), len(added), len(removed), len(changed)) != (153, 0, 1, 0):
        errors.append("expected 153 unchanged + 0 added + 1 removed + 0 changed")
    if removed != ["jei.chat.error.command.too.long"]:
        errors.append(f"unexpected removed keys: {removed}")
    ...
    for token in (
        ...
        "forgeVersion=42.0.0",
        "parchmentVersionForge=1.18.2-2022.07.10-1.19.1",
        "specificationVersion=11.2.0",
    ):
        if token not in props:
            errors.append(f"pinned G29 gradle.properties missing {token}")
    if 'languageVersion.set(JavaLanguageVersion.of(modJavaVersion))' not in forge_build:
        errors.append("pinned G29 Forge build metadata no longer confirms Java toolchain")
    if 'mappings("parchment", parchmentVersionForge)' not in forge_build:
        errors.append("pinned G29 Forge build metadata no longer confirms Parchment mappings")
    ...
    if len(inherited) != 86:
        errors.append(f"expected G28 selected scope 86, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.19"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.19.1"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.19/1.19.1 missing from Mojang version manifest")
    ...
    print("Minecraft 1.19.1 / JEI 11.2.0 final localization exploratory audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print("Build: Minecraft 1.19.1 / Forge 42.0.0 / Parchment 1.18.2-2022.07.10-1.19.1 / Java 17")
    print(f"G28 English: {len(base)} total / {len(base_normal)} normal / {len(base)-len(base_normal)} debug")
    print(f"G29 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G28 -> G29 unchanged: {len(unchanged)}")
    print(f"G28 -> G29 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G28 -> G29 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G28 -> G29 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    ...
    print("Upstream completeness against normal G29 target keys:")
    for locale in UPSTREAM_LOCALES:
    ...
    if base_meta and target_meta:
        ...
        print(f"Minecraft 1.19 asset index: id={ba.get('id')} sha1={ba.get('sha1')} url={ba.get('url')}")
        print(f"Minecraft 1.19.1 asset index: id={ta.get('id')} sha1={ta.get('sha1')} url={ta.get('url')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1')}")
    print(f"Minecraft language asset-file codes: 1.19={len(base_codes)} 1.19.1={len(target_codes)}")
    print(f"Added asset-file codes since 1.19 ({len(added_mc)}): {', '.join(added_mc) or '(none)'}")
    print(f"Removed asset-file codes since 1.19 ({len(removed_mc)}): {', '.join(removed_mc) or '(none)'}")
    ...
    print(f"Inherited selected codes absent from 1.19.1 ({len(inherited_missing)}): {', '.join(inherited_missing) or '(none)'}")
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.19.1 / JEI 11.2.0 exploratory audit")
    return 0
