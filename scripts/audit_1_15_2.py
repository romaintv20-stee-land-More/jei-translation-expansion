#!/usr/bin/env python3
"""Audit final Minecraft 1.15.2 / JEI 6.0.2 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.15.1-language-scope.json"
DIFF_PATH = ROOT / "upstream" / "diffs" / "1.15.1-to-1.15.2.json"
PINNED_COMMIT = "1ea77203d8b731c99cea68166b02afeb3d9c6176"
...
def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G18-audit"})
...
def main() -> int:
    ...
    if stored_target != remote_target:
        errors.append("stored G18 English source differs from pinned JEI 6.0.2 endpoint")
    ...
    if actual_counts != frozen_counts:
        errors.append(f"frozen G17->G18 diff counts {frozen_counts} differ from live endpoint {actual_counts}")
    if added != diff["added_keys"]:
        errors.append(f"frozen G18 added-key set {diff['added_keys']} differs from live endpoint {added}")
    ...
    if len(inherited) != 87:
        errors.append(f"expected G17 selected scope of 87, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((item for item in manifest.get("versions", []) if item.get("id") == "1.15.1"), None)
    target_entry = next((item for item in manifest.get("versions", []) if item.get("id") == "1.15.2"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.15.1/1.15.2 missing from Mojang version manifest")
    ...
    print("Minecraft 1.15.2 / JEI 6.0.2 final localization audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print(f"G17 English: {len(base)} total")
    print(f"G18 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G17 -> G18 unchanged: {len(unchanged)}")
    print(f"G17 -> G18 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G17 -> G18 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G17 -> G18 changed ({len(changed)}):")
    for key in changed:
        print(f"  {key}: {base[key]!r} -> {target[key]!r}")
    print(f"JEI upstream JSON locales ({len(upstream_locales)}): {', '.join(upstream_locales)}")
    print("Upstream completeness against normal G18 target keys:")
    for locale in upstream_locales:
    ...
    if base_meta and target_meta:
        ...
        print(f"Minecraft 1.15.1 asset index: id={ba.get('id')} sha1={ba.get('sha1')} url={ba.get('url')}")
        print(f"Minecraft 1.15.2 asset index: id={ta.get('id')} sha1={ta.get('sha1')} url={ta.get('url')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1') and ba.get('url') == ta.get('url')}")
    print(f"Minecraft language codes: 1.15.1={len(base_codes)} 1.15.2={len(target_codes)}")
    print(f"Added codes since 1.15.1 ({len(added_mc)}): {', '.join(added_mc) or '(none)'}")
    print(f"Removed codes since 1.15.1 ({len(removed_mc)}): {', '.join(removed_mc) or '(none)'}")
    print(f"Inherited selected codes absent from 1.15.2 ({len(inherited_missing)}): {', '.join(inherited_missing) or '(none)'}")
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned final Minecraft 1.15.2 / JEI 6.0.2 audit")
    return 0
