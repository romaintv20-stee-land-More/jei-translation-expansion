#!/usr/bin/env python3
"""Audit the pinned Minecraft 1.11.2 / JEI 4.5.1 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.11-language-scope.json"
...
MC_1_11_ASSET_INDEX = (
    ...
    "c64959fe73672e9b053b157f57b6aba318d0b3b5/1.11.json"
)
DEBUG_PREFIX = "description.jei."


def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G8-audit"})
...
def main() -> int:
    ...
    if parse_lang_text(remote_english) != target:
        errors.append("stored 1.11.2 English source differs from pinned JEI upstream")
    ...
    if len(selected) != 72:
        errors.append(f"expected inherited G7 selected scope of 72, got {len(selected)}")

    manifest = fetch_json(VERSION_MANIFEST)
    version_entry = next((item for item in manifest.get("versions", []) if item.get("id") == "1.11.2"), None)
    if not version_entry:
        errors.append("Minecraft 1.11.2 missing from Mojang version manifest")
        target_meta = {}
        target_asset = {}
    else:
    ...
    if inherited_missing_from_mc:
        errors.append(
            "inherited selected G7 languages absent from Minecraft 1.11.2: "
    ...
    print("Minecraft 1.11.2 / JEI 4.5.1 localization audit")
    ...
    print(f"G7 -> G8 unchanged: {len(unchanged)}")
    print(f"G7 -> G8 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G7 -> G8 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G7 -> G8 changed ({len(changed)}):")
    for key in changed:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned Minecraft 1.11.2 / JEI 4.5.1 audit")
    return 0
