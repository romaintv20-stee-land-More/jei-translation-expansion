#!/usr/bin/env python3
"""Audit final Minecraft 1.14.2 / JEI 6.0.0 localization endpoint."""
...
BASE_SCOPE = ROOT / "upstream" / "minecraft-1.13.2-language-scope.json"
PINNED_COMMIT = "f1fd2f1d20cf86d9644e907a641f99873b4d8888"
...
def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G14-audit"})
...
def language_metadata(asset_index: dict) -> dict[str, dict]:
    ...
    if not obj or "hash" not in obj:
        return {}
    sha1 = obj["hash"]
    raw = fetch_json(f"https://resources.download.minecraft.net/{sha1[:2]}/{sha1}")
...
def main() -> int:
    ...
    if len(inherited) != 87:
        errors.append(f"expected G13 selected scope of 87, got {len(inherited)}")

    manifest = fetch_json(VERSION_MANIFEST)
    base_entry = next((item for item in manifest.get("versions", []) if item.get("id") == "1.13.2"), None)
    target_entry = next((item for item in manifest.get("versions", []) if item.get("id") == "1.14.2"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.13.2/1.14.2 missing from Mojang version manifest")
    ...
    candidates = sorted(target_codes - inherited)
    ...
    print("Minecraft 1.14.2 / JEI 6.0.0 localization audit")
    print(f"Pinned commit: {PINNED_COMMIT}")
    print(f"G13 English: {len(base)} total")
    print(f"G14 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G13 -> G14 unchanged: {len(unchanged)}")
    print(f"G13 -> G14 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G13 -> G14 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G13 -> G14 changed ({len(changed)}):")
    for key in changed:
        print(f"  {key}: {base[key]!r} -> {target[key]!r}")

    print(f"JEI upstream JSON locales ({len(upstream_locales)}): {', '.join(upstream_locales)}")
    print("Upstream completeness against normal G14 target keys:")
    for locale in upstream_locales:
    ...
    if base_meta and target_meta:
        ...
        print(f"Minecraft 1.13.2 asset index: id={ba.get('id')} sha1={ba.get('sha1')} url={ba.get('url')}")
        print(f"Minecraft 1.14.2 asset index: id={ta.get('id')} sha1={ta.get('sha1')} url={ta.get('url')}")
        print(f"Asset index identical: {ba.get('sha1') == ta.get('sha1') and ba.get('url') == ta.get('url')}")
    print(f"Minecraft language codes: 1.13.2={len(base_codes)} 1.14.2={len(target_codes)}")
    print(f"Added codes since 1.13.2 ({len(added_mc)}): {', '.join(added_mc) or '(none)'}")
    print(f"Removed codes since 1.13.2 ({len(removed_mc)}): {', '.join(removed_mc) or '(none)'}")
    print(f"Inherited selected codes absent from 1.14.2 ({len(inherited_missing)}): {', '.join(inherited_missing) or '(none)'}")
    print(f"Inherited selected codes still present: {len(inherited_present)}")
    print(f"Minecraft 1.14.2 codes outside inherited scope ({len(candidates)}):")
    for code in candidates:
        info = target_language_metadata.get(code, {})
        print(f"  {code}: name={info.get('name', '?')!r} region={info.get('region', '?')!r} bidirectional={info.get('bidirectional', False)}")
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: pinned Minecraft 1.14.2 / JEI 6.0.0 audit")
    return 0
