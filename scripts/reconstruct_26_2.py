#!/usr/bin/env python3
"""Reconstruct Minecraft 26.2 / JEI 30.32.0 language resources deterministically."""
...
DEFAULT_OUTPUT = ROOT / "build" / "reconstructed" / "26.2"
DEBUG_PREFIX = "description.jei."
G51_COMMIT = "f93563ca4965d511bd07d4f041b3a6ddd1158ef0"
...
parse_json = g50.parse_json
write_json = g50.write_json
preserves_runtime_literals = g50.preserves_runtime_literals
...
def fetch_locale(commit: str, locale: str) -> dict[str, str]:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "JEI-Translation-Expansion-G51-reconstruct"})
    try:
...
def fetch_g51_upstream(locale: str) -> dict[str, str]:
    values = fetch_locale(G51_COMMIT, locale)
    if not values:
        raise ValueError(f"{locale}: missing or invalid pinned G51 upstream locale")
    return values


@lru_cache(maxsize=1)
def g50_scope() -> dict:
    return json.loads(g50.SCOPE_PATH.read_text(encoding="utf-8"))


@lru_cache(maxsize=1)
def g50_full() -> dict[str, dict[str, str]]:
    base, target = g50.verify_semantics()
    values, _ = g50.reconstruct_full(base, target, g50_scope())
    return values


@lru_cache(maxsize=1)
def g50_supplements() -> dict[str, dict[str, str]]:
    base, target = g50.verify_semantics()
    values, _ = g50.reconstruct_supplements(base, target, g50_scope())
    return values


@lru_cache(maxsize=None)
def g50_combined_locale(locale: str) -> dict[str, str]:
    scope = g50_scope()
    full = g50_full()
    if locale in full:
    ...
    if locale in set(scope["selected_upstream_complete_locales"]):
        if locale == "en_us":
            return parse_json(g50.TARGET_SOURCE)
        return g50.fetch_g50_upstream(locale)
    if locale in set(scope["selected_upstream_incomplete_locales"]):
        combined = dict(g50.fetch_g50_upstream(locale))
        combined.update(g50_supplements()[locale])
        return combined
    raise KeyError(f"{locale}: not selected in G50")
...
def verify_semantics() -> tuple[dict[str, str], dict[str, str]]:
    ...
    if (len(base), len(target), len(normal)) != (334, 334, 328):
        raise ValueError("G51 English source counts changed")
    if (len(unchanged), len(added), len(removed), len(changed)) != (334, 0, 0, 0):
        raise ValueError("G51 semantic delta changed")
    return base, target
...
def reconstruct_full(base: dict[str, str], target: dict[str, str], scope: dict) -> tuple[dict[str, dict[str, str]], dict[str, dict[str, int]]]:
    ...
    if len(expected) != 63 or len(fallback) != 30 or not fallback <= expected:
        raise ValueError("G51 full/fallback ownership changed")

    result: dict[str, dict[str, str]] = {}
    stats: dict[str, dict[str, int]] = {}
    for locale in sorted(expected):
        previous = g50_combined_locale(locale)
        values: dict[str, str] = {}
        reused = 0
        english = 0
...
def reconstruct_supplements(base: dict[str, str], target: dict[str, str], scope: dict) -> tuple[dict[str, dict[str, str]], dict[str, dict[str, int]]]:
    ...
    if len(expected) != 26:
        raise ValueError("G51 supplement ownership changed")

    result: dict[str, dict[str, str]] = {}
    stats: dict[str, dict[str, int]] = {}
    for locale in sorted(expected):
        upstream = fetch_g51_upstream(locale)
        ...
        for key in override_keys:
            if key not in normal or key not in upstream or preserves_runtime_literals(target[key], upstream[key]):
                raise ValueError(f"{locale}: invalid/unneeded G51 safety override {key}")
        ...
        if not values or (set(values) & set(upstream)) != override_keys:
            raise ValueError(f"{locale}: invalid G51 supplement ownership")
...
def reconstruct_all(output: Path, clean: bool = True) -> tuple[int, int, int, dict]:
    ...
    if (
        ...
        raise ValueError("G51 frozen diff changed")
    ...
    if not reuse.get("reuse_only_exact_same_key_same_english") or reuse.get("cross_key_reuse_allowed") is not False:
        raise ValueError("G51 reuse policy changed")
    ...
    if (len(full), len(supplements), len(complete)) != (63, 26, 1) or complete != {"en_us"}:
        raise ValueError("G51 ownership total changed")
    ...
    provenance = {
        ...
        "upstream_commit": G51_COMMIT,
        "cross_key_reuse_allowed": False,
        "full_locales": full_stats,
        "supplement_locales": supplement_stats,
        "totals": {
            "g50-exact-semantic": sum(x["g50-exact-semantic"] for x in full_stats.values()) + sum(x["g50-exact-semantic"] for x in supplement_stats.values()),
...
def main() -> int:
    ...
    print("PASS: Minecraft 26.2 deterministic reconstruction")
    ...
    print(f"Exact G50 semantic reuses: {provenance['totals']['g50-exact-semantic']}")
    print(f"Exact target-English values emitted: {provenance['totals']['target-English']}")
    print("All 334 G51 English semantics are identical to G50")
