#!/usr/bin/env python3
"""Exploratory audit for Minecraft 1.21.6 / JEI 22.0.0."""
...
def fetch_bytes(url: str, timeout: int = 30) -> bytes:
    headers = {"User-Agent": "JEI-Translation-Expansion-G42-audit"}
...
def main() -> int:
    ...
    for token in (
        ...
        "minecraftVersionRange=[1.21.6, 1.21.7)",
        "neoforgeVersion=21.6.20-beta",
        "neoforgeVersionRange=[21.6.20-beta,)",
        "specificationVersion=22.0.0",
    ):
        if token not in props:
            errors.append(f"pinned G42 gradle.properties missing {token}")
    ...
    base_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.21.5"), None)
    target_entry = next((x for x in manifest.get("versions", []) if x.get("id") == "1.21.6"), None)
    if not base_entry or not target_entry:
        errors.append("Minecraft 1.21.5/1.21.6 missing from Mojang manifest")
    ...
    if absent:
        errors.append(f"selected inherited locales absent from Minecraft 1.21.6 assets: {absent}")
    ...
    print("Minecraft 1.21.6 / JEI 22.0.0 exploratory audit")
    print(f"Pinned endpoint: {PINNED_COMMIT}")
    print(f"Next Minecraft port: 1.21.7 at {NEXT_PORT_COMMIT}")
    print("Endpoint is final because the 1.21.7 port directly follows this commit on mainline.")
    print("Build: Minecraft 1.21.6 / NeoForge 21.6.20-beta / Java 21 / JEI specification 22.0.0")
    print(f"G41 English: {len(base)} total / {len(base_normal)} normal / {len(base)-len(base_normal)} debug")
    print(f"G42 English: {len(target)} total / {len(target_normal)} normal / {len(target)-len(target_normal)} debug")
    print(f"G41 -> G42 unchanged: {len(unchanged)}")
    print(f"G41 -> G42 added ({len(added)}): {', '.join(added) or '(none)'}")
    print(f"G41 -> G42 removed ({len(removed)}): {', '.join(removed) or '(none)'}")
    print(f"G41 -> G42 changed ({len(changed)}): {', '.join(changed) or '(none)'}")
    for key in changed:
        print(f"  changed {key}: {base[key]!r} -> {target[key]!r}")
    if base_meta and target_meta:
        ba, ta = base_meta["assetIndex"], target_meta["assetIndex"]
        print(f"Minecraft 1.21.5 asset index: id={ba.get('id')} sha1={ba.get('sha1')}")
        print(f"Minecraft 1.21.6 asset index: id={ta.get('id')} sha1={ta.get('sha1')}")
    print(f"Minecraft live language codes: 1.21.5={len(base_codes)} 1.21.6={len(target_codes)}")
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: Minecraft 1.21.6 / JEI 22.0.0 exploratory audit")
    return 0
